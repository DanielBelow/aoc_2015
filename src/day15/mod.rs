...
fn calculate_score(
    ...
    for (num, key) in ingredients {
        let ing_data = data.get(*key).unwrap();
...
fn find_best_score(data: &HashMap<String, IngredientData>, pred: fn(&[i64; 5]) -> bool) -> i64 {
    let mut result = 0;
...
#[aoc(day15, part1)]
pub fn part1(data: &HashMap<String, IngredientData>) -> i64 {
    find_best_score(data, part1_pred)
}

#[aoc(day15, part2)]
pub fn part2(data: &HashMap<String, IngredientData>) -> i64 {
...
mod tests {
    ...
    fn get_test_score(gen: &HashMap<String, IngredientData>, pred: fn(&[i64; 5]) -> bool) -> i64 {
        (0..=100)
            .map(|bs| {
                let c = 100 - bs;
